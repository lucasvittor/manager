...
def tela_principal():
    layout = [
        [sg.Text("", size=(30, 1), justification="center", key='-DATA_HORA-', font=("Helvetica", 14), expand_x=True)],
...
# Atualizar a data e hora na janela principal
    if main_window:
        agora = datetime.now()
        data_hora_atual = agora.strftime("%d/%m/%Y %H:%M:%S")
        main_window['-DATA_HORA-'].update(data_hora_atual)
